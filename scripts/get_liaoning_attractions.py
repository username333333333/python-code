...
def get_liaoning_attractions(api_key, page_size=20, max_pages=50):
    """
    使用高德地图API获取辽宁省旅游景点数据
    ...
    for page in range(1, max_pages + 1):
        ...
        try:
            ...
            print(f"正在处理第{page}页，获取到{len(pois)}个景点")
    ...
    print(f"数据获取完成，共获取到{len(all_attractions)}个景点")
    return pd.DataFrame(all_attractions)
...
if __name__ == "__main__":
    ...
    # 获取数据
    df = get_liaoning_attractions(API_KEY)
