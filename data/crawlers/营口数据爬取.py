...
# 写入表头
with open('营口2013-2023年天气数据.csv', mode='w', encoding='utf-8', newline='') as f:
    writer = csv.writer(f)
    writer.writerow([
        '最高气温',
        '最低气温',
...
def fetch_month_data(year, month, retries=3):
    url = f"http://www.tianqihoubao.com/lishi/yingkou/month/{year}{month:02d}.html"
    headers = {
            ...
            for row in rows:
                ...
                if len(weather)==2 and len(temps)==2 and len(winds)==2:
                    ...
                    if len(parts_day)==2 and len(parts_night)==2:
                        ...
                        with open('营口2013-2023年天气数据.csv', mode='a', encoding='utf-8', newline='') as f:
